...
class QuestViewer(tk.Tk):
    ...
    def ingest_boxes(self, raw: list):
        ...
        for parent, desc, ul, lr, done in raw:
            self.box_groups.setdefault(parent, []).append(dict(desc=desc, ul=tuple(ul), lr=tuple(lr), done=bool(done)))
        for w in self.group_chk_frame.winfo_children():
            w.destroy()
            self.group_vars.clear()
        for parent in sorted(self.box_groups):
            var = tk.BooleanVar(value=True)
            ttk.Checkbutton(self.group_chk_frame, text=parent, variable=var, command=self.redraw_boxes).pack(anchor="w")
    ...
    def _grid_to_canvas(self,gx:int,gy:int)->Tuple[float,float]:
        cw,ch=self.orig_im.width/GRID_DIVISIONS,self.orig_im.height/GRID_DIVISIONS
        return self.offset_x+gx*cw*self.scale, self.offset_y+gy*ch*self.scale
